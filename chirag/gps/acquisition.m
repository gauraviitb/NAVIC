function [sat_detect, code_phase_aq, f_doppler, acquisition_mat, phase, peak_value] = acquisition(fs, fi, prn, T, fid, T_off )
...
M       = size(fd_lo,2);    % M = no of freq bin to search , m = size(X,dim) returns the size of the dimension of X specified by scalar dim.

code_lo = 2*repmat(cacode(prn,fs/1.023e6),1,T)-1;  
...
peak_threshold  = 3000; % should be changed as fs changes or input signal power changes
% Decide peak threshold for a given false alarm proablity (pg 254 GPS: Theory and Applications, volume 1)
...
%% Acquisition
    ...
    I_lo = cos(2*pi*(fi + fd_lo)'*(1/fs:1/fs:T*1e-3));    % transpose and multiply
...
while lock==0 && size(rec_sig,1)== fs*T*1e-3 &&  ms_count<10
    ...
    for v = 1:L
        temp_I = I_lo.*repmat( circshift(code_lo,code_phi_lo(v),2),M,1);
        temp_Q = Q_lo.*repmat( circshift(code_lo,code_phi_lo(v),2),M,1);     %right circular shift
...
if  lock==1
    ...
    % Calculate phase at the peak (optional)%%%%%%%%%%%%%%%%%%%%%
    temp_I = cos(2*pi*(fi + f_doppler)*(1/fs:1/fs:T*1e-3)).*circshift(code_lo,code_phase_aq,2);
    temp_Q = sin(2*pi*(fi + f_doppler)*(1/fs:1/fs:T*1e-3)).*circshift(code_lo,code_phase_aq,2);
    I      = temp_I*rec_sig_I' + temp_Q*rec_sig_Q';
    Q      = -temp_Q*rec_sig_I' + temp_I*rec_sig_Q';
    phase  = atan(I/Q);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    disp(['Acqusition Complete. Satellite ', prn, ' found'])
else
    sat_detect = 0;
    f_doppler  = 0;
    code_phase_aq = 0;
