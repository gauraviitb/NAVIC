...
freq_bin   = 100; % As T is increased freq_bin should reduce 
freq_range = 5e3; 
...
peak_threshold  = 35000*T*fs/8e6; % should be changed as fs changes or input signal power changes
...
while lock==0 && size(rec_sig,1)== fs*T*1e-3 &&  ms_count<10
    ...
    for v = 1:L
        ...
        temp_Q = Q_lo.*repmat( circshift(code_lo,code_phi_lo(v),2),M,1);
    ...
    if abs(max_index - max_index_old)< 2*code_bin || peak_value >= peak_threshold 
        lock = 1;
    end
