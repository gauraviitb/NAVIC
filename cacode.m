...
% force integers
testint=round(sv)-sv;   %round off to nearest integers
if testint ~= 0 
...
tap_sel=tap(sv,:);
for inc=1:L
    g2(:,inc)=mod(sum(q(tap_sel),2),2);   %mod(x,y) is x - n.*y where n = floor(x./y)     Sum = column vector containing sum of each row
